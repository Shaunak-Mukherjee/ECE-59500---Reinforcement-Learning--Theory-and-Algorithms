# 🧠 ECE 59500 — Reinforcement Learning: Theory and Algorithms

> Purdue University — Graduate Course Repository  
> Term: Fall 2025  
> Language: Python 3.10+  

---

## 🚀 Overview

This repository contains assignments, implementations, and notes for **ECE 59500 – Reinforcement Learning: Theory and Algorithms**, a graduate-level course focusing on theoretical foundations and algorithmic aspects of RL.


Simulation Platforms information provided in coursework:

OpenAI Gym
https://gymnasium.farama.org/ 
Unity ML-Agents
https://unity.com/products/machine-learning-agents 
Griddly: A grid-world game engine
https://griddly.readthedocs.io/en/latest/index.html 
Libraries:

RLlib
https://docs.ray.io/en/latest/rllib/index.html 
The Minigrid library
https://github.com/Farama-Foundation/Minigrid 
Other Resources:

Hugging Face Deep RL Course
https://huggingface.co/learn/deep-rl-course/unit0/introduction 
15 Python Reinforcement Learning Project Ideas for Beginners
https://www.projectpro.io/article/reinforcement-learning-projects-ideas-for-beginners-with-code/521
RL Theory seminars
https://sites.google.com/view/rltheoryseminars 
Collection of research papers for model-based reinforcement learning
https://github.com/opendilab/awesome-model-based-RL
